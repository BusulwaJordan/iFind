...
enum _AttachOption { photo, camera, location, file, quote, product }
...
class _ChatRoomBodyState extends ConsumerState<_ChatRoomBody> {
  ...
  Future<void> _pickAndUpload(ImageSource source) async {
    ...
    try {
      ...
      final url = await storage.uploadXFile(
        bucket: 'product_images',
        xFile: xFile,
        path: 'chat-images/${widget.chat.id}',
      );
      await ref.read(chatRemoteDataSourceProvider).sendMessage(
            chatId: widget.chat.id,
            senderId: user.id,
            content: '[CHAT_IMAGE]::$url',
          );
      ref.invalidate(myChatsProvider);
      WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToBottom());
    } catch (e) {
      if (mounted) {
        AppToast.show(context, 'Could not send image: ${friendlyError(e)}',
  ...
  Future<void> _shareLocation() async {
    ...
    try {
      ...
      if (mounted) {
        AppToast.show(context, 'Could not share location: ${friendlyError(e)}',
            type: ToastType.error);
      }
    } finally {
      if (mounted) setState(() => _isUploading = false);
        ...
        onSend: (items, total) async {
          ...
          try {
            await ref.read(chatRemoteDataSourceProvider).sendMessage(
                  ...
                  senderId: user.id,
                  content:
                      'IFIND_QUOTATION::${jsonEncode(<String, dynamic>{"items": items, "total": total, "status": "pending"})}',
                );
            ref.invalidate(myChatsProvider);
            WidgetsBinding.instance
                .addPostFrameCallback((_) => _scrollToBottom());
  ...
  void _handleAttach(_AttachOption option) {
    switch (option) {
      case _AttachOption.photo:
      case _AttachOption.file:
        _pickAndUpload(ImageSource.gallery);
      case _AttachOption.camera:
        _pickAndUpload(ImageSource.camera);
      case _AttachOption.location:
        _shareLocation();
      case _AttachOption.quote:
        _showQuotationBuilder();
      case _AttachOption.product:
        break;
    }
  }
...
class _MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isChatImage = msg.content.startsWith('[CHAT_IMAGE]::');
    ...
    final chatImageUrl = isChatImage
        ? msg.content.substring('[CHAT_IMAGE]::'.length)
        : null;
    final locationCoords =
        isLocation ? msg.content.substring('[LOCATION]::'.length) : null;
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              if (isQuotation && quotationData != null)
                ...
                Flexible(
                  child: GestureDetector(
                    onTap: () => _openMedia(
                        context, chatImageUrl, 'image', null, msg.createdAt),
                    child: Container(
              ...
              else if (isLocation && locationCoords != null)
                Flexible(
                  child: GestureDetector(
                    onTap: () {
...
class _InputArea extends StatelessWidget {
  ...
  final void Function(_AttachOption) onAttach;
    ...
    return Container(
      ...
      child: SafeArea(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            // ── Attachment icons row — all 6 spread across full width ──
            Row(
              ...
              children: [
                if (isUploading)
                  ...
                  _AttachBtn(
                    ...
                    label: 'Photo',
                    onTap: () => onAttach(_AttachOption.photo),
                  ),
                _AttachBtn(
                  icon: Icons.camera_alt_outlined,
                  label: 'Camera',
                  onTap: () => onAttach(_AttachOption.camera),
                ),
                _AttachBtn(
                  icon: Icons.location_on_outlined,
                  label: 'Location',
                  onTap: () => onAttach(_AttachOption.location),
                ),
                _AttachBtn(
                  icon: Icons.inventory_2_outlined,
                  label: 'Product',
                  onTap: () => onAttach(_AttachOption.product),
                ),
                _AttachBtn(
                  icon: Icons.description_outlined,
                  label: 'File',
                  onTap: () => onAttach(_AttachOption.file),
                ),
                _AttachBtn(
                  icon: Icons.attach_money_rounded,
                  label: 'Quote',
                  onTap: () => onAttach(_AttachOption.quote),
                ),
              ],
            ),
            const SizedBox(height: 8),
...
class _QuotationBuilderSheet extends StatefulWidget {
  final void Function(String items, num total) onSend;
  const _QuotationBuilderSheet({required this.onSend});

  @override
  State<_QuotationBuilderSheet> createState() =>
      _QuotationBuilderSheetState();
}
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Items / Description',
              style:
                  GoogleFonts.outfit(fontSize: 13, color: Colors.grey[600])),
          const SizedBox(height: 6),
          TextField(
            controller: _itemsCtrl,
            style: GoogleFonts.outfit(),
            decoration: InputDecoration(
              hintText: 'e.g. 30 Pairs Nike Air Max',
            ...
            child: ElevatedButton(
              ...
              onPressed: () {
                ...
                final total = num.tryParse(
                    _totalCtrl.text.trim().replaceAll(',', ''));
                if (items.isEmpty || total == null) return;
                widget.onSend(items, total);
              },
